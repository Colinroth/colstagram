...
//  Colstagram
//
//  Created by Colin M. Roth on 7/4/15.
...
-(void)viewWillLayoutSubviews {
    ...
    if (CGRectGetHeight(self.view.frame) > 500) {
        ...
        filterViewHeight = CGRectGetHeight(self.view.frame) - CGRectGetMaxY(self.view.frame) - buffer - buffer;
...
-(void) addFiltersToQueue {
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *moodyFilter = [CIFilter filterWithName:@"CISRCBToneCurveToLinear"];
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        CIFilter *tiltFilter = [CIFilter filterWithName:@"CIStriaghtenFilter"];
...
- (void) sendButtonPressed: (id)sender {
    ...
    if ([[UIApplication sharedApplication] canOpenURL:instagramURL]) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"No Instagram App", nil) message:NSLocalizedString(@"The Instagram app isn't installed on your device. Please install it from the App Stor.e", nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"Ok", @"Ok button") otherButtonTitles:nil];
        [alert show];
    }
}
...
-(void) alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (buttonIndex != alertView.cancelButtonIndex) {
        ...
        NSURL *fileURL = [[tmpDirURL URLByAppendingPathComponent:@"Colstagram"] URLByAppendingPathExtension:@"igo"];
        
        BOOL success = [imageData writeToURL:fileURL atomically:YES];
        
        if (!success) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Couldn't save image", nil) message:NSLocalizedString(@"Your photo couldn't be saved. Maked sure you have enough disk space and try again.", nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"Ok", @"Ok button") otherButtonTitles: nil];
            [alert show];
            return;
        }
        ...
        if (caption.length > 0) {
            self.documentController.annotation = @{@"InstagramCation": caption};
        }
